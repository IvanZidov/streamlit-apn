...
CIJENA = st.sidebar.number_input(
    "Cijena nekretnine (€): ", value=100000, min_value=0, step=500
)
KAPARA = st.sidebar.number_input("Kapara (€): ", value=10000, min_value=0, step=500)
GODINA_POTPORE = 0
MJESTO = ""
VISINA_SUBVENCIJE = 0
...
with st.sidebar.expander("Dodatne postavke"):
    ...
    IZNOS_KREDITA = st.number_input(
        "Iznos kredita: ", value=round(CIJENA - KAPARA), min_value=0
    )
    TECAJ_EURA = st.number_input(
        "Tečaj eura: ", value=7.51, format="%.2f", min_value=0.00
    )
...
if VRSTA_KREDITA_1 == "APN":
    KAMATA_1 = stupac1.number_input(
        "Kamata (%): ", value=2.10, format="%.2f", min_value=0.00
    )
else:
    KAMATA_1 = stupac1.number_input(
        "Kamata (%): ", value=2.90, format="%.2f", min_value=0.00
    )
TRAJANJE_1 = stupac1.number_input("Godina otplate: ", 15, 30, value=15)

VRSTA_KREDITA_2 = stupac2.radio(
    "Vrsta kredita:", ("APN", "Običan stambeni"), key="2", index=1
)
if VRSTA_KREDITA_2 == "APN":
    KAMATA_2 = stupac2.number_input(
        "Kamata (%): ", value=2.10, format="%.2f", min_value=0.00, key="2"
    )
else:
    KAMATA_2 = stupac2.number_input(
        "Kamata (%): ", value=2.90, format="%.2f", min_value=0.00, key="2"
    )
TRAJANJE_2 = stupac2.number_input("Godina otplate: ", 15, 30, value=20, key="2")
...
class Kredit:
    ...
    def calculate_credit(self):
        ...
        MJESECNA_RATA = npf.pmt(
            self.KAMATA / 100 / 12, self.TRAJANJE * 12, -(self.IZNOS_KREDITA)
        )
    ...
    def show_results(self, container):
        ...
        container.metric(
            "Ukupni iznos potpore:", value=str(self.UKUPNI_IZNOS_POTPORE) + " €"
        )
        ...
        with container.expander("Više informacija"):
            ...
            st.metric(
                ...
                value=str(round(self.CIJENA * 0.005, 2)) + " €",
            )
            ...
            if self.VRSTA_KREDITA == "APN":
                ...
                st.metric(
                    ...
                    value=str(round(self.MJESECNA_RATA_APN * self.TECAJ_EURA, 2))
                    + " kn",
                    delta="-"
                    + str(round(self.MJESECNA_POTPORA * self.TECAJ_EURA, 2))
                    + " kn",
                    delta_color="inverse",
                )
                st.metric(
            ...
            st.metric(
                "Anuitet u kunama",
                value=str(round(self.MJESECNA_RATA * self.TECAJ_EURA, 2)) + " kn",
            )
            st.metric(
                "Anuitet u eurima", value=str(round(self.MJESECNA_RATA, 2)) + " 💶"
            )
...
razlika = (
    CREDIT_SUMMARY["UKUPNA_CIJENA_NEKRETNINE"]
    - CREDIT_SUMMARY2["UKUPNA_CIJENA_NEKRETNINE"]
)
if razlika > 0:
    prvidrugi = 1
else:
...
if prvidrugi == -1:
    st.header("Prvi kredit je ukupno povoljniji za:")
    st.header(str(round(prvidrugi * razlika, 2)) + " €")
else:
    st.header("Drugi kredit je ukupno povoljniji za:")
    st.header(str(round(prvidrugi * razlika, 2)) + " €")
    ...
    c = (
        ...
        .encode(
            ...
            tooltip=[
                ...
                "MJESECNA_RATA",
                "MJESECNA_RATA_APN",
                "UKUPNI_IZNOS_POTPORE",
                "UKUPNA_CIJENA_NEKRETNINE",
            ],
        )
    )
...
st.write("---")
html_script = """<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
<style>
.fa {
  padding: 20px;
  font-size: 30px;
...
st.markdown("""Ivan Židov<a href="https://www.linkedin.com/in/ivan-zidov/" class="fa fa-linkedin"></a>
